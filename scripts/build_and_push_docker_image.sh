...
docker image build . --no-cache -t "$(whoami)/dependency-scanner:$version"

if [ -n "${TAG_AND_PUSH}" ]
then
    echo "Tagging for production and saving"
    docker image tag "$(whoami)/dependency-scanner:$version" "eu.gcr.io/bu-fyp-s5008913/dependency-scanner:$version"
    docker push "eu.gcr.io/bu-fyp-s5008913/dependency-scanner:$version"
fi